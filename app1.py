...
class GeminiApi:
    ...
    def generate_embeddings(self, texts: List[str]) -> np.ndarray:
        ...
        return self.embedder.encode(texts, convert_to_numpy=True, batch_size=32)
...
class QueryRetrievalSystem:
    ...
    def index_document(self, document: Document):
        """Index document clauses using FAISS with HNSW."""
        ...
        self.index = faiss.IndexHNSWFlat(embeddings.shape[1], 32)  # 32 is the number of links per node
        self.index.hnsw.efConstruction = 200  # Higher efConstruction for better index quality
        self.index.hnsw.efSearch = 64  # Higher efSearch for faster search
        self.index.add(embeddings)

    def search_clauses(self, query: str, top_k: int = 5) -> List[Tuple[str, str, float]]:
        """Retrieve top-k relevant clauses using FAISS HNSW search."""
    ...
    def process_query(self, query: str, document: Document) -> Dict:
        ...
        matched_clauses = self.search_clauses(query, top_k=5)
